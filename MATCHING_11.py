...
X_simulador = np.load('datos_simulador.npy').reshape(-1, 1)
y_ensayos = np.load('resultados_ensayos.npy').reshape(-1, 1)
...
X_simulador = scaler_X.fit_transform(X_simulador)
y_ensayos = scaler_y.fit_transform(y_ensayos)
...
y_pred_normalized = model.predict(X_simulador_normalized)

# Invertir la normalización para obtener las predicciones en la escala original
y_pred = scaler_y.inverse_transform(y_pred_normalized)

plt.figure(figsize=(8, 6))
plt.scatter(X_simulador, y_ensayos, label='Datos de entrenamiento', alpha=0.5)
plt.scatter(X_simulador, y_pred, color='red', label='Predicciones', alpha=0.5)
plt.xlabel('X')
plt.ylabel('Y')
